...
export default function CompliancePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const overdue = ((actions as any[]) || []).filter(x => x.status !== "done" && x.due_date && new Date(x.due_date) < new Date()).length;
      ...
      const doneCount = COMPLIANCE_STEPS.filter(s => state[s.key]).length;
  ...
  async function setAnnualDone(v: boolean) {
    ...
    // refresh
    const newDone = { ...done, annual: v };
    setDone(newDone);
    const doneCount = COMPLIANCE_STEPS.filter(s => newDone[s.key]).length;
    setScore(Math.round((doneCount / COMPLIANCE_STEPS.length) * 100));
  }
          ...
          <div className="card" style={{ padding: 16 }} key={s.key}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
              <div>
                ...
                <div className="small" style={{ marginTop: 6 }}>{s.why}</div>
                <div className="small" style={{ marginTop: 6 }}><span className="badge">{s.lawHint}</span></div>
              </div>
              <div>
                {done[s.key] ? <span className="badge ok">Klar</span> : <span className="badge warn">Ej klar</span>}
              </div>
            </div>

            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 12 }}>
              {s.key === "policy" && <Link className="btn btn-primary" href="/app/policy">Skapa policy</Link>}
              {s.key === "risk" && <Link className="btn btn-primary" href="/app/riskbedomningar/ny">Skapa riskbedömning</Link>}
              {s.key === "actions" && <Link className="btn btn-primary" href="/app/atgarder">Se åtgärder</Link>}
              {s.key === "incidents" && <Link className="btn btn-primary" href="/app/incidenter/rapportera">Rapportera incident</Link>}
              {s.key === "annual" && (
                <button className="btn btn-primary" onClick={() => setAnnualDone(true)}>
                  Markera årlig uppföljning klar
                </button>
              )}
              {s.key === "annual" && done[s.key] && (
